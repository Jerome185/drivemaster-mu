export const translations = {
  fr: {
    premiumTitle: "Passe ton permis plus rapidement",
    premiumSubtitle: "Réussis ton examen avec confiance",

    official: "Officiel",
    officialDesc: "Parfait pour débutants",

    master: "Master",
    masterDesc: "Réussis ton examen avec confiance",

    o1: "Questions illimitées",
    o2: "Pratique à tout moment",
    o3: "Améliore tes bases",
    o4: "Mode Master",

    m1: "Tout dans Officiel",
    m2: "Questions Master",
    m3: "Questions difficiles",
    m4: "Simulation réelle",

    upgradeOfficial: "Passer à Officiel",
    upgradeMaster: "Passer à Master",
    currentPlan: "Plan actuel",

    paymentTitle: "Paiement Juice",
    sendTo: "Envoyez le paiement à :",
    amount: "Montant",
    transaction: "Transaction ID",
    pay: "J’ai payé",
    cancel: "Annuler",
    sending: "Envoi...",

    enterTx: "Veuillez entrer le Transaction ID",
    paymentPending: "Paiement soumis ✅ En attente",
    paymentError: "Erreur lors du paiement",

    loading: "Chargement...",

    // HERO
    title: "Réussis ton permis à Maurice du premier coup",
    subtitle: "Entraîne-toi avec des questions type examen + explications claires",
    start: "Commencer maintenant",
    offer: "🔥 Offre lancement : Rs 999 (places limitées)",

    feature1: "Français & Anglais",
    feature2: "Questions réalistes",
    feature3: "Mobile friendly",

    // PROBLEM
    problemTitle: "Pourquoi beaucoup échouent ?",
    problem1: "Questions piégeuses",
    problem2: "Réponses ambiguës",
    problem3: "Pas assez d’entraînement réel",
    problemConclusion: "Ce n’est pas un problème de conduite.",

    // SOLUTION
    solutionTitle: "La solution : DriveMaster MU",
    sol1Title: "Questions type examen",
    sol1Desc: "Inspirées des vrais tests",
    sol2Title: "Explications simples",
    sol2Desc: "Comprends tes erreurs",
    sol3Title: "Mobile friendly",
    sol3Desc: "Révise partout",

    // CTA
    ctaTitle: "Prêt à réussir ton permis ?",
    ctaButton: "Accéder aux examens",
    ctaSub: "Accès immédiat après inscription",

    learning: "Learning",
    dashboard: "Dashboard",
    admin: "Admin",
    login: "Connexion",
    logout: "Déconnexion",
    next: "Suivant",
    retry: "Réessayer",
    examfinished: "Examen terminé",
    score: "Score",
    success: "Bien joué !",
    fail: "Continue à t'entraîner !"
  },

  en: {
    premiumTitle: "Upgrade your driving skills",
    premiumSubtitle: "Pass your test with confidence",

    official: "Official",
    officialDesc: "Perfect for beginners",

    master: "Master",
    masterDesc: "Pass your test with confidence",

    o1: "Unlimited questions",
    o2: "Practice anytime",
    o3: "Improve basics",
    o4: "No Master mode",

    m1: "Everything in Official",
    m2: "Master questions",
    m3: "Hard questions",
    m4: "Real exam simulation",

    upgradeOfficial: "Upgrade to Official",
    upgradeMaster: "Upgrade to Master",
    currentPlan: "Current Plan",

    paymentTitle: "Juice Payment",
    sendTo: "Send payment to:",
    amount: "Amount",
    transaction: "Transaction ID",
    pay: "I have paid",
    cancel: "Cancel",
    sending: "Sending...",

    enterTx: "Please enter Transaction ID",
    paymentPending: "Payment submitted ✅ Pending approval",
    paymentError: "Payment error",

    loading: "Loading...",

    // HERO
    title: "Pass your driving test in Mauritius on your first try",
    subtitle: "Practice with real exam questions + clear explanations",
    start: "Start now",
    offer: "🔥 Launch offer: Rs 999 (limited spots)",

    feature1: "French & English",
    feature2: "Realistic questions",
    feature3: "Mobile friendly",

    // PROBLEM
    problemTitle: "Why do many fail?",
    problem1: "Tricky questions",
    problem2: "Confusing answers",
    problem3: "Not enough real practice",
    problemConclusion: "It’s not a driving problem.",

    // SOLUTION
    solutionTitle: "The solution: DriveMaster MU",
    sol1Title: "Real exam questions",
    sol1Desc: "Inspired by real tests",
    sol2Title: "Simple explanations",
    sol2Desc: "Understand your mistakes",
    sol3Title: "Mobile friendly",
    sol3Desc: "Study anywhere",

    // CTA
    ctaTitle: "Ready to pass your test?",
    ctaButton: "Access exams",
    ctaSub: "Instant access after signup",

     learning: "Learning",
     dashboard: "Dashboard",
     admin: "Admin",
     login: "Login",
     logout: "Logout",
     next: "Suivant",
     retry: "Réessayer",
     examfinished: "Exam finished",
     score: "Score",
     success: "Well done!",
     fail: "Keep practicing!"
}

  }